class BulletObj{
    ...
    adjustPath(theta, health){ // the speed at which the bullet is drawn toward a player is inversely proportional to the root of the player's health
        speed = 100/Math.sqrt(health);
        vec = [this.speed*Math.cos(this.angle) + speed*Math.cos(this.angle), this.speed*Math.sin(this.angle) + speed*Math.sin(this.angle)];
        this.angle = Math.tan(vec[1]/vec[0]); // update angle
        this.speed += speed/0.95; // TODO: rewrite for projection calculations
    }

    move(){
        this.position = [this.position[0] + this.speed*Math.cos(this.angle), this.position[1] + this.speed*Math.sin(this.angle)];
        this.speed *= 0.95;
    }
...
class Player{
    ...
    shoot(theta){ // mouse direction
        this.#health--;
        this.#bullets.push(new BulletObj(this.#base[0] + (this.#radius() + this.#speed())*Math.cos(theta), this.#base[1] + (this.#radius + this.#speed())*Math.sin(theta), theta));
    }

    /**
     * @param {Number} theta
     */
    #move(theta){ // pi/4 increments because if like up & left are pressed, we get a 3pi/4 angle --- keyboard directions
        if(theta !== undefined){
    ...
    getBlocks(){
        return [for(bullet of this.#bullets) bullet.getPosition()];
    }
}
